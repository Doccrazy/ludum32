...
public class UiItemSlots extends Widget {
	...
	public void act(float delta) {
	    ...
	    world.pollEvents(NewAbilityEvent.class, (NewAbilityEvent event) -> {
	        if (player.getAbility(1) instanceof StartAbility) {
	            player.setAbility(1, event.getAbility());
	        } else {
	...
	public void draw(Batch batch, float parentAlpha) {
		...
		if (world.isGameInProgress()) {
		    int x = pendingAbility != null ? 30 : 0;
			batch.draw(Resource.GFX.itemSlot, Constants.SCREEN_WIDTH / 2 - 130 - x, Constants.SCREEN_HEIGHT - 120, 100, 100);
			if (player.getAbility(0) != null) {
				batch.setColor(1, 1, 1, player.isCooldown(0, FireMode.PRIMARY) ? 0.5f : 1);
				batch.draw(player.getAbility(0).getTexture(FireMode.PRIMARY), Constants.SCREEN_WIDTH / 2 - 130 - x, Constants.SCREEN_HEIGHT - 120, 100, 100);
				batch.setColor(1, 1, 1, 1);
			}
			batch.draw(Resource.GFX.itemSlot, Constants.SCREEN_WIDTH / 2 + 30 + x, Constants.SCREEN_HEIGHT - 120, 100, 100);
			if (player.getAbility(1) != null) {
				batch.setColor(1, 1, 1, player.isCooldown(1, FireMode.ALTERNATE) ? 0.5f : 1);
				batch.draw(player.getAbility(1).getTexture(FireMode.ALTERNATE), Constants.SCREEN_WIDTH / 2 + 30 + x, Constants.SCREEN_HEIGHT - 120, 100, 100);
				batch.setColor(1, 1, 1, 1);
			}

			if (pendingAbility != null) {
			    batch.setColor(1, 1, 1, abilityTime % 0.5f < 0.25f ? 1 : 0);
	            batch.draw(Resource.GFX.itemSlot, Constants.SCREEN_WIDTH / 2 - 50, Constants.SCREEN_HEIGHT - 120, 100, 100);
                batch.draw(pendingAbility.getTexture(FireMode.PRIMARY), Constants.SCREEN_WIDTH / 2 - 50, Constants.SCREEN_HEIGHT - 120, 100, 100);
                batch.setColor(1, 1, 1, 1);
			} else {
